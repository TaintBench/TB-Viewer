...
export async function activate(context: ExtensionContext) {
	...
	client.onNotification("taintbench/groundtruth", handleTreeDataNotification(groundtruthProvider, "taint.groundtruth", true, false));
	client.onNotification("taintbench/aqlresults", handleTreeDataNotification(aqlresultsProvider, "taint.aqlresults", false, true));
	client.onNotification("taintbench/matchedresults", handleTreeDataNotification(matchedresultsProvider, "taint.matchedresults", true, true));
	client.onNotification("taintbench/unmatchedresults", handleTreeDataNotification(unmatchedresultsProvider, "taint.unmatchedresults", false, true));
...
export class SimpleTreeDataProvider implements TreeDataProvider<TreeViewNode> {
	...
	truePositives: Array<TreeViewNode> = new Array();
	falsePositives: Array<TreeViewNode> = new Array();
	triageRequired: Array<TreeViewNode> = new Array();
	...
	update(uri: string, diagnostics: Diagnostic[], viewId: String, seperation: boolean, isDetected: boolean) {
			...
			if (seperation) {
				if (d.severity === DiagnosticSeverity.Error)
					...
					if (d.message.includes("P].")) {
						this.truePositives.push(flowNode);
					} else if (d.message.includes("N].")) {
						this.falsePositives.push(flowNode);
					}
				}
			} else {
		...
		if (seperation) {
			...
			this.truePositives.sort((n1, n2) => {
				var id1 = parseInt(n1.label.split("[")[1].split("P]")[0]);
				var id2 = parseInt(n2.label.split("[")[1].split("P]")[0]);
				return id1 - id2;
			}
			);

			this.falsePositives.sort((n1, n2) => {
				var id1 = parseInt(n1.label.split("[")[1].split("N]")[0]);
				var id2 = parseInt(n2.label.split("[")[1].split("N]")[0]);
				return id1 - id2;
			}
			);
		}
		...
		if (seperation) {
			if (!isDetected) {
				...
				summaryPositive.label = "True Positives: " + this.truePositives.length;
				summaryPositive.children = this.truePositives;
				summaryPositive.iconPath = path.join(__filename, '..', '..', 'media/summary.svg');
				summaryPositive.collapsibleState = TreeItemCollapsibleState.Expanded;

				summaryNegative.label = "False Positives: " + this.truePositives.length;
				summaryNegative.children = this.falsePositives;
				summaryNegative.iconPath = path.join(__filename, '..', '..', 'media/summary.svg');
				summaryNegative.collapsibleState = TreeItemCollapsibleState.Expanded;
			}
		}
...
export function handleTreeDataNotification(dataProvider: SimpleTreeDataProvider, viewId: string, seperation: boolean, isDetected: boolean) {
	return (args: PublishDiagnosticsParams) => {
		if (dataProvider)
			dataProvider.update(args.uri, args.diagnostics, viewId, seperation, isDetected);
	}
}
